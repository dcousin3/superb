...
The package ``superb`` facilites the production of 
summary statistics having the correct error bars. Error bars are
measures of precisions; they are meant to convey some indications of
...
trustable that those with short error bars. 

Error bars (and measures of precision) comes in many flavors. There is
...
95% confidence intervals; this can be changed, as see in Vignette 5. We begin with data
format.
...
a list with ---for the moment--- an adjustment for the purpose of the plot: ``purpose =
 "difference"``. The standalone CI is the default (it can be obtained explicitely with 
...
The reader is refered to the package ``ggplot2`` for more on these graphing directives.
...
* ``LM``: this method, the first developped by Loftus and Masson [-@lm94], will 
...
of sphericity indicates rejection of sphericity, so interprete the error bars with caution.
...
adustment) and CRS (adjustments that tend to widen the error bars) are the
...
These adjustments are implemented in ``superb`` with additional adjustements, such as
