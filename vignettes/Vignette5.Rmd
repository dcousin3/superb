...
superbPlot.simple <- function( summarydata, xvar, groupingfac, addfactors, rawdata ) {
    plot <- ggplot(
        data = summarydata,
...
superbPlot.simple <- function(
    summarydata, xvar, groupingfac, addfactors, rawdata 
) {
    plot <- ggplot(
        data = summarydata, 
        mapping = aes_string( x = xvar, y = "center")
...
superbPlot.empty <- function(
    ...
    runDebug( 'inempty', "Getting the dataframes",
        c("summary","raw"), list(summarydata, rawdata))

    plot <- ggplot() # an empty plot        
    return(plot)
}
options(superb.debug = 'inempty')  ## turn on Debug when entering plot function
superbPlot(TMB1964r,
     WSFactor = "T(7)",      
     BSFactor = "Condition",
     variables = c("T1","T2","T3","T4","T5","T6","T7"),
