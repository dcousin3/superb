...
before begining the classes. This study most likely will have two groups, one where
...
Taking into accound the confidence interval, the manipulation seems to improve
...
examine, say, the firt group to the value 100. As this value is outside the interval,
...
the t-test), there is a simple adjustment that can be brougth to the error bars: just
...
by additing an adjustment to the list of adjustments with ``adjustments = list(purpose = 
...
This is where the usefullness of ``superb`` is apparent: it only required an option to 
...
A second way to compare the two plots is to superimpse them, as in Figure 4:
