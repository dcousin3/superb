...
png(filename = "Figure1.png", width = 1280, height = 640)
grid.arrange(plt1a, plt1b, plt1c, ncol=3)
dev.off()
```
...
```{r, message=FALSE, echo=TRUE, eval=TRUE, fig.width=3, fig.height=4, fig.cap="Figure 2c: Making right panel of Figure 2."}
...
Reusing ``ornateBS``, we get an unadjusted plot as usual:
...
plt3a = superbPlot(dataFigure3, 
    ...
    plotStyle   = "line" )
plt3a <- plt3a + ornateBS + labs(subtitle="Stand-alone\n95% CI") 
```
...
plt3b <- plt3b + ornateBS + labs(subtitle="Cluster and difference-\nadjusted 95% CI")
```
...
plt3c <- plt3c + ornateBS + labs(subtitle="Cluster and difference-\nadjusted 95% CI")
```
...
```{r, message=FALSE, echo=TRUE, eval=FALSE}
png(filename = "Figure3.png", width = 1280, height = 640)
grid.arrange(plt3a, plt3b, plt3c, ncol=3)
dev.off()
```
...
plt4b <- superbPlot(dataFigure4, 
    ...
    plotStyle="line" ) 
plt4b <- plt4b + ornateBS + labs(subtitle="Population size and difference-\nadjusted 95% CI") 
```
...
plt4c <- superbPlot(dataFigure4, 
    ...
    violinParams = list(fill = "green", alpha = 0.2)  ) 
plt4c <- plt4c + ornateBS + labs(subtitle="Population size and difference-\nadjusted 95% CI") 
```
...
plt5a = superbPlot(dataFigure1, BSFactor = "grp", 
    statistic = "var",
    adjustments=list(purpose = "difference"),
  ...
  geom_hline(yintercept = 100, colour = "black", size = 0.5, linetype=2)
plt5b = superbPlot(dataFigure1, BSFactor = "grp",
...
plt6a <- superbPlot(dataFigure1, BSFactor = "grp", 
...
plt6b <- superbPlot(dataFigure1, BSFactor = "grp", 
