...
As seen from the message, the correlations in nearby time points is about .80. It explains why the 
...
a significant result. As an example, consider the green curve, in condition cue+stimuli (bottom
...
If you look at the red curve (cue alone) on times 5 and 6, no difference is suggested, and so 
says the t-test:
...
You can vary the radius from 1 to 9 (it cannot exceed the number of time point minus 1, divided by 2).
The larger the radius, the smallest will be the benefit of correlation in the assessment of precision.
...
Local decorrelation is a tool adapted to time series where nearby measurements are expected to show greater
correlations than measurements separated by large amount of time.
