...
```{r, fig.width=6.7, fig.height=2.5, fig.cap="Figure 2, preliminary version"}
...
```{r, fig.width=6.7, fig.height=2.5, fig.cap="Figure 2, version with colors"}
...
commonstyle <- list(
    ...
    # set the labels to be displayed (superb replace them with consecutive numbers...)
...
```{r, fig.width=6.7, fig.height=2.5, fig.cap="Figure 2, final version"}
...
graphic directive, put options regarding specifically this second data set (smaller y-axis range, different
label, etc.):

```{r, fig.width=6.7, fig.height=2.5, fig.cap="Figure 2, bottom row"}
...
Finally, we assemble the two plots toghether

```{r, fig.width=6.7, fig.height=5.0, fig.cap="Figure 2, final version"}
...
ggsave( "Figure2.png",
    plot=finalplt,
    device = "png",
    dpi = 320,          # pixel per inches
    units = "cm",
    width = 17,         # as per the article
    height = 13
)
```
